...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    /*
     * Esta bandera solamente se considera verdadera
     * cuando JavaScript la envía explícitamente como "1".
     */
    $eliminarImagen =
    ...
    $urlImagenActual =
        !empty($vacanteActual['url_imagen'])
    ...
    /*
     * Por defecto se conserva exactamente la imagen actual.
     *
     * Esto es lo importante para que modificar únicamente
     * título, texto, fecha o estado NO afecte la imagen.
     */
    ...
    /* =====================================================
       CASO 2: SE SOLICITÓ ELIMINAR LA IMAGEN
       ===================================================== */
    ...
    if (
        ...
        /*
         * Si acabamos de subir una imagen pero el registro
         * no puede continuar, eliminamos la nueva imagen.
         */

        if (
            $rutaImagenNueva &&
            file_exists($rutaImagenNueva)
        ) {
    ...
    if (!$stmt) {
         ...
         * no pudo prepararse, eliminamos esa imagen.
         */
    ...
    if (!$stmt->execute()) {
        ...
        /*
         * Si la BD falló después de guardar una imagen nueva,
         * eliminamos la nueva imagen para evitar archivos
         * huérfanos.
         */
    ...
    /*
     * Solamente eliminamos la imagen anterior cuando:
     *
     * 1. Se reemplazó por una nueva.
     * 2. Se eliminó explícitamente.
     *
     * Si solamente se editó texto, no entramos aquí.
     */
    ...
    if (
        ...
        $rutaRealAnterior =
            realpath($rutaImagenActual);

        $rutaRealNueva =
            $rutaImagenNueva
                ? realpath($rutaImagenNueva)
                : null;

        if (
            !$rutaRealNueva ||
            $rutaRealAnterior !== $rutaRealNueva
        ) {
            @unlink(
                $rutaImagenActual
            );
